...
function KubernetesObject({ position, isSelected, onClick, totalPods, warningCount, unhealthyPodCount, podStatusCounts, appsSynced, appsTotal, kubeletVersion }: {
  position: [number, number, number];
  isSelected?: boolean;
  onClick?: () => void;
  totalPods?: number;
  ...
  return (
    <Float speed={1.9} floatIntensity={0.4}>
      ...
      >
        ...
        {kubeletVersion && (
          <Text position={[0, -0.9, 0]} fontSize={0.07} color="#4b5563" anchorX="center"
            // @ts-expect-error
            toneMapped={false}
          >{kubeletVersion}</Text>
        )}
        ...
        {isSelected && (
          <Html position={[0.8, 0.3, 0]} style={{ pointerEvents: "none" }}>
            <div style={{
              ...
              <div style={{ color: k8sColor, marginBottom: 5, fontWeight: 600 }}>Kubernetes {kubeletVersion ?? ""}</div>
